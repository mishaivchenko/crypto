...
class AutoApprovalEvaluatorTest {
    ...
    void returnsEmptyWhenNoRules() {
        Optional<AutoApprovalEvaluator.RuleMatch> result = AutoApprovalEvaluator.evaluate(
                candidate("gate", "BTC/USDT", new BigDecimal("0.10")), null, null, List.of());
        assertThat(result).isEmpty();
    }

    @Test
    void matchesFirstRuleByPriority() {
        AutoApprovalRule lowPriority = rule(
                200,
                AutoApprovalAction.AUTO_REJECT,
                new BigDecimal("0.05"),
                null,
                List.of(),
                List.of(),
                null,
                List.of());
        AutoApprovalRule highPriority = rule(
                10,
        ...
        Optional<AutoApprovalEvaluator.RuleMatch> result = AutoApprovalEvaluator.evaluate(
                candidate("gate", "BTC/USDT", new BigDecimal("0.10")), null, null, List.of(lowPriority, highPriority));
    ...
    void rejectsWhenFundingRateBelowMin() {
        ...
        Optional<AutoApprovalEvaluator.RuleMatch> result = AutoApprovalEvaluator.evaluate(
                candidate("gate", "BTC/USDT", new BigDecimal("0.05")), null, null, List.of(rule));
        assertThat(result).isEmpty();
    }
    ...
    void rejectsWhenFundingRateAboveMax() {
        ...
        Optional<AutoApprovalEvaluator.RuleMatch> result = AutoApprovalEvaluator.evaluate(
                candidate("gate", "BTC/USDT", new BigDecimal("0.50")), null, null, List.of(rule));
        assertThat(result).isEmpty();
    }
    ...
    void rejectsWhenVenueNotAllowed() {
        AutoApprovalRule rule =
                rule(1, AutoApprovalAction.AUTO_EXECUTE, null, null, List.of("bybit"), List.of(), null, List.of());

        Optional<AutoApprovalEvaluator.RuleMatch> result = AutoApprovalEvaluator.evaluate(
                candidate("gate", "BTC/USDT", new BigDecimal("0.10")), null, null, List.of(rule));
        assertThat(result).isEmpty();
    }
    ...
    void matchesWhenVenueAllowed() {
        AutoApprovalRule rule =
                rule(1, AutoApprovalAction.AUTO_EXECUTE, null, null, List.of("gate"), List.of(), null, List.of());
    ...
    void rejectsWhenConfidenceBelowThreshold() {
        AutoApprovalRule rule =
                rule(1, AutoApprovalAction.AUTO_EXECUTE, null, null, List.of(), List.of(), 0.8, List.of());
    ...
    void noConditionsRuleMatchesEverything() {
        AutoApprovalRule rule =
                rule(1, AutoApprovalAction.AUTO_EXECUTE, null, null, List.of(), List.of(), null, List.of());

        Optional<AutoApprovalEvaluator.RuleMatch> result =
                AutoApprovalEvaluator.evaluate(candidate(null, "BTC/USDT", null), null, null, List.of(rule));
        assertThat(result).isPresent();
    }
    ...
    void resolvesVenueFromHintsWhenSourceVenueAbsent() {
        ...
        Optional<AutoApprovalEvaluator.RuleMatch> result =
                AutoApprovalEvaluator.evaluate(candidateWithHint, null, null, List.of(rule));
        assertThat(result).isPresent();
    }
