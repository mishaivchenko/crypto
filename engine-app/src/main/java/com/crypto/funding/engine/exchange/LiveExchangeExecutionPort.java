...
public class LiveExchangeExecutionPort implements ExecutionPort {
    ...
    public LiveExchangeExecutionPort(
            Environment environment, com.crypto.funding.engine.EngineProperties engineProperties) {
        this(environment, engineProperties, null, HttpClient.newHttpClient(), new ObjectMapper());
    }

    public LiveExchangeExecutionPort(
            Environment environment,
            com.crypto.funding.engine.EngineProperties engineProperties,
    ...
    protected LiveExchangeExecutionPort(
            Environment environment,
            com.crypto.funding.engine.EngineProperties engineProperties,
            EngineCredentialCache credentialCache,
    ...
    public OrderAttempt submitOrder(EngineExecutionPlan plan, OrderIntent intent, boolean reduceOnly) {
        ...
        try {
            ...
            return failed(
                    ...
                    intent,
                    attemptedAt,
                    "Unsupported live order venue: " + normalizedVenue);
            ...
            return failed(
                    ...
                    intent,
                    attemptedAt,
                    "Live order submission interrupted: " + e.getMessage());
        } catch (Exception e) {
            return failed(
                    plan,
                    normalizedVenue,
                    ...
                    intent,
                    attemptedAt,
                    "Live order submission failed: " + e.getMessage());
        }
    }

    private OrderAttempt submitBybit(
            EngineExecutionPlan plan, OrderIntent intent, boolean reduceOnly, Instant attemptedAt)
    ...
    private OrderAttempt bybitStatus(
            EngineExecutionPlan plan, OrderIntent intent, BigDecimal quantity, String orderId, Instant attemptedAt)
            throws IOException, InterruptedException {
        String baseUrl = baseUrl("bybit");
        String query =
                "category=linear&symbol=" + encode(plan.venueSymbol()) + "&orderId=" + encode(orderId) + "&openOnly=1";
        long timestamp = System.currentTimeMillis();
        String apiKey = credential("bybit", "api-key");
        String secretKey = credential("bybit", "secret-key");
        ...
        JsonNode order = root.path("result").path("list").isArray()
                        && !root.path("result").path("list").isEmpty()
    ...
    private OrderAttempt submitGate(
        ...
        if (response.statusCode() >= 300) {
            return rejected(
                    ...
                    quantity,
                    attemptedAt,
                    "Gate order rejected: " + root.path("message").asText(response.body()));
        }
    ...
    private OrderAttempt submitOkx(
        ...
        boolean isTestnet = "testnet"
                .equals(property("engine.trading-venue-access-mode", "testnet")
                        .trim()
    ...
    private OrderAttempt submitKucoin(
        ...
        String sign = HmacSigner.hmacSha256Base64(secretKey, signPayload);
        String passphraseEncoded = com.crypto.funding.crypto.HmacSigner.hmacSha256Base64(
                secretKey, rawPassphrase == null ? "" : rawPassphrase);
        ...
        if (response.statusCode() >= 300 || !"200000".equals(code)) {
            return rejected(
                    ...
                    quantity,
                    attemptedAt,
                    "Kucoin order rejected: " + root.path("msg").asText(response.body()));
        }
    ...
    private OrderAttempt submitBitget(
        ...
        HttpRequest request = bitgetBuilder
                .POST(HttpRequest.BodyPublishers.ofString(bodyJson))
                .build();
        ...
        if (response.statusCode() >= 300 || !"00000".equals(code)) {
            return rejected(
                    ...
                    quantity,
                    attemptedAt,
                    "Bitget order rejected: " + root.path("msg").asText(response.body()));
        }
        String orderId = root.path("data").path("orderId").asText();
    ...
    private OrderAttempt bitgetStatus(
        ...
        String query =
                "symbol=" + encode(plan.venueSymbol()) + "&orderId=" + encode(orderId) + "&productType=USDT-FUTURES";
        String timestamp = String.valueOf(System.currentTimeMillis());
        String signPayload = timestamp + "GET" + requestPath + "?" + query;
        String sign = HmacSigner.hmacSha256Base64(secretKey, signPayload);
    ...
    private String liveGateFailure(
            EngineExecutionPlan plan, String venue, OrderIntent intent, boolean reduceOnly, Instant now) {
        if (plan == null) {
        ...
        if (plan.venueSymbol() == null
                || plan.venueSymbol().isBlank()
    ...
    private BigDecimal liveMarketPrice(EngineExecutionPlan plan) throws IOException, InterruptedException {
        ...
        if ("okx".equals(venue)) {
            String url =
                    baseUrl("okx") + "/api/v5/public/mark-price?instType=SWAP&instId=" + encode(plan.venueSymbol());
        ...
        if ("kucoin".equals(venue)) {
            ...
            return requirePositiveDecimal(
                    root.path("data").path("value").asText(null), "Kucoin mark price value is missing.");
        }
        if ("bitget".equals(venue)) {
            String url = baseUrl("bitget") + "/api/v2/mix/market/symbol-price?symbol=" + encode(plan.venueSymbol())
                    + "&productType=USDT-FUTURES";
            JsonNode root = getJson(url, "Bitget mark price");
            JsonNode data = root.path("data");
            JsonNode item = data.isArray() && !data.isEmpty() ? data.get(0) : objectMapper.createObjectNode();
            return requirePositiveDecimal(
                    item.path("markPrice").asText(null), "Bitget mark price markPrice is missing.");
        }
    ...
    private BigDecimal contractMultiplier(EngineExecutionPlan plan) throws IOException, InterruptedException {
        ...
        if ("gate".equals(venue)) {
            ...
            return requirePositiveDecimal(
                    root.path("quanto_multiplier").asText(null), "Gate contract quanto_multiplier is missing.");
        }
        if ("okx".equals(venue)) {
            String url =
                    baseUrl("okx") + "/api/v5/public/instruments?instType=SWAP&instId=" + encode(plan.venueSymbol());
    ...
    private OrderAttempt rejected(
            EngineExecutionPlan plan, OrderIntent intent, BigDecimal quantity, Instant attemptedAt, String reason) {
        return new OrderAttempt(
                null,
                null,
                plan.armedTradeId(),
    ...
    private String missingCredentialsReason(String venue) {
        ...
        if (apiKey == null || apiKey.isBlank() || secretKey == null || secretKey.isBlank()) {
            return "Missing engine credentials for " + venue + ". Add API key and secret for " + venue
                    + " in the monitor venue settings.";
        }
        if (requiresPassphrase(venue)) {
            String passphrase = credential(venue, "passphrase");
            if (passphrase == null || passphrase.isBlank()) {
                return "Missing engine passphrase for " + venue + ". Add passphrase for " + venue
                        + " in the monitor venue settings.";
            }
        }
        return null;
    ...
    private String baseUrl(String venue) {
        String mode =
                property("engine.trading-venue-access-mode", "testnet").trim().toLowerCase(Locale.ROOT);
    ...
    private Set<String> enabledVenues() {
        return Arrays.stream(
                        property("engine.live-enabled-venues", "bybit,gate").split(","))
    ...
    private boolean stale(Instant sampledAt, long maxAgeMinutes, Instant now) {
        return sampledAt == null
                || sampledAt.plus(Duration.ofMinutes(maxAgeMinutes)).isBefore(now);
    }
