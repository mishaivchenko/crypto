...
public class EngineTelemetryService {
    ...
    public void recordOrderSubmission(String venue, OrderAttemptStatus status, long durationMs) {
        ...
        submitStatsByVenue
                .computeIfAbsent(normalizeVenue(venue), ignored -> new VenueSubmitStats())
    ...
    private Map<String, Long> attemptStatusSnapshot() {
        ...
        attemptStatusBreakdown.forEach(
                (status, count) -> snapshot.put(status.name().toLowerCase(Locale.ROOT), count.get()));
