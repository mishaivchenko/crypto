...
class CredentialAwareExecutionPortTest {
    ...
    void liveGateRejectsWhenLiveFlagIsOff() {
        MockEnvironment environment = liveEnvironment("bybit").withProperty("engine.live-order-enabled", "false");
        CredentialAwareExecutionPort port = new CredentialAwareExecutionPort(environment);
    ...
    void liveBybitAcceptsMultipleEntryAttempts() {
        MockEnvironment environment = liveEnvironment("bybit").withProperty("engine.live-order-enabled", "true");
        CredentialAwareExecutionPort port = new CredentialAwareExecutionPort(environment);
    ...
    void submitsBybitLinearMarketSellAndReadsFilledStatus() {
        exchange.stubFor(get(urlEqualTo("/v5/market/tickers?category=linear&symbol=REQUSDT"))
                .willReturn(okJson("""
        ...
        exchange.stubFor(post("/v5/order/create").willReturn(okJson("""
            ...
            """)));
        exchange.stubFor(
                get(urlEqualTo("/v5/order/realtime?category=linear&symbol=REQUSDT&orderId=bybit-order-1&openOnly=1"))
                        .willReturn(okJson("""
                ...
                """)));
        CredentialAwareExecutionPort port = new CredentialAwareExecutionPort(liveEnvironment("bybit")
    ...
    void submitsGateEntryUsingTickerAndContractMultiplier() {
        exchange.stubFor(
                get(urlEqualTo("/futures/usdt/tickers?contract=REQ_USDT")).willReturn(okJson("""
        ...
        exchange.stubFor(post("/futures/usdt/orders").willReturn(okJson("""
            ...
            """)));
        CredentialAwareExecutionPort port = new CredentialAwareExecutionPort(liveEnvironment("gate")
                .withProperty("engine.live-order-enabled", "true")
    ...
    void liveGateRejectsWhenLatencyProfileIsStale() {
        ...
        CredentialAwareExecutionPort port = new CredentialAwareExecutionPort(environment);
        EngineExecutionPlan stale =
                livePlanWithTimings("bybit", Instant.now(), Instant.now().minusSeconds(1441 * 60));
    ...
    void submitsGateMarketReduceOnlyExit() {
        ...
        var attempt = port.submitOrder(
                livePlan("gate"),
                new OrderIntent(TradeSide.LONG, ExecutionType.MARKET, BigDecimal.TEN, null, null),
                true);
    ...
    private static EngineExecutionPlan livePlan(
            String venue, Integer entryAttemptCount, BigDecimal positionEntryPrice) {
    ...
    private static EngineExecutionPlan livePlanWithTimings(
            String venue, Instant metadataLastSyncedAt, Instant latencySampledAt) {
        return new EngineExecutionPlan(
                5L,
                10L,
                venue,
    ...
    void submitsOkxEntryWithSimulatedTradingHeaderInTestnet() {
        exchange.stubFor(get(urlEqualTo("/api/v5/public/mark-price?instType=SWAP&instId=REQ-USDT-SWAP"))
                .willReturn(okJson("""
            {"code":"0","data":[{"instId":"REQ-USDT-SWAP","markPx":"2.5"}]}
            """)));
        exchange.stubFor(get(urlEqualTo("/api/v5/public/instruments?instType=SWAP&instId=REQ-USDT-SWAP"))
                .willReturn(okJson("""
        ...
        exchange.stubFor(post("/api/v5/trade/order").willReturn(okJson("""
            ...
            """)));
        exchange.stubFor(get(com.github.tomakehurst.wiremock.client.WireMock.urlPathEqualTo("/api/v5/trade/order"))
                .willReturn(okJson("""
            {"code":"0","data":[{"ordId":"okx-order-1","state":"filled","fillPx":"2.5","accFillSz":"10","fee":"-0.01"}]}
            """)));
        CredentialAwareExecutionPort port = new CredentialAwareExecutionPort(liveEnvironmentWithPassphrase("okx")
                .withProperty("engine.live-order-enabled", "true")
        ...
        assertThat(attempt.averageFillPrice()).isEqualByComparingTo("2.5");
        exchange.verify(
                com.github.tomakehurst.wiremock.client.WireMock.postRequestedFor(urlEqualTo("/api/v5/trade/order"))
    ...
    void submitsOkxReduceOnlyExit() {
        ...
        exchange.verify(
                postRequestedFor(urlEqualTo("/api/v5/trade/order")).withRequestBody(containing("\"side\":\"buy\"")));
    }
    ...
    void submitsKucoinEntryMarketSell() {
        ...
        exchange.stubFor(get(urlEqualTo("/api/v1/orders/kucoin-order-1")).willReturn(okJson("""
            ...
            """)));
        CredentialAwareExecutionPort port = new CredentialAwareExecutionPort(liveEnvironmentWithPassphrase("kucoin")
                .withProperty("engine.live-order-enabled", "true")
    ...
    void submitsBitgetEntryMarketSell() {
        exchange.stubFor(get(urlEqualTo("/api/v2/mix/market/symbol-price?symbol=REQUSDT&productType=USDT-FUTURES"))
                .willReturn(okJson("""
        ...
        exchange.stubFor(post("/api/v2/mix/order/place-order").willReturn(okJson("""
            ...
            """)));
        exchange.stubFor(get(com.github.tomakehurst.wiremock.client.WireMock.urlPathEqualTo("/api/v2/mix/order/detail"))
                .willReturn(okJson("""
            {"code":"00000","data":{"state":"filled","fillPrice":"2.5","baseVolume":"10","fee":"0.01"}}
            """)));
        CredentialAwareExecutionPort port = new CredentialAwareExecutionPort(liveEnvironmentWithPassphrase("bitget")
                .withProperty("engine.live-order-enabled", "true")
