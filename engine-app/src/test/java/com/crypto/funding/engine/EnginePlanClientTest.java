...
class EnginePlanClientTest {
    ...
    void setUp() {
        ...
        client = new EnginePlanClient(
                builder, properties(), telemetryService, advancingNanos(2_000_000_000L, 40_000_000L));
    }
    ...
    void postsPositionStateAndOutcomeLifecycleContracts() {
        ...
        var position = client.recordPosition(new EnginePositionRecordRequest(
                ...
                null));
        var state =
                client.updateTradeState(5L, new EngineTradeStateUpdateRequest(ArmedTradeState.OPEN, "entry filled"));
    ...
    void omitsInternalTokenHeaderWhenTokenIsBlank() {
        RestClient.Builder builder = RestClient.builder();
        MockRestServiceServer blankTokenServer =
        ...
        EnginePlanClient blankTokenClient = new EnginePlanClient(
                builder, properties, new EngineTelemetryService(), advancingNanos(3_000_000_000L, 40_000_000L));

        blankTokenServer
                .expect(requestTo("http://monitor.test/internal/v1/engine/plans?includeAll=false"))
                .andExpect(method(HttpMethod.GET))
                .andExpect(request -> assertThat(request.getHeaders().containsKey("X-Internal-Token"))
                        .isFalse())
                .andRespond(withSuccess("[]", MediaType.APPLICATION_JSON));
    ...
    void omitsInternalTokenHeaderWhenTokenIsNull() {
        RestClient.Builder builder = RestClient.builder();
        MockRestServiceServer nullTokenServer =
                MockRestServiceServer.bindTo(builder).build();
        EngineProperties properties = properties();
        properties.setInternalToken(null);
        EnginePlanClient nullTokenClient = new EnginePlanClient(
                builder, properties, new EngineTelemetryService(), advancingNanos(4_000_000_000L, 40_000_000L));
    ...
    void postsWarmupCalibrationToCorrectPathWithInternalToken() {
        ...
        client.recordWarmupCalibration(
                7L,
                new com.crypto.funding.contract.engine.WarmupCalibrationRequest(
