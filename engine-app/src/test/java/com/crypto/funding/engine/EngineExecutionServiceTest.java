...
class EngineExecutionServiceTest {
    ...
    void setUp() {
        service = new EngineExecutionService(
                ...
                clock,
                advancingNanos(
                        Duration.ofSeconds(1).toNanos(), Duration.ofMillis(25).toNanos()));
    ...
    void skipsFutureTriggersWhenNotForced() {
        when(client.listPlans(false))
                .thenReturn(List.of(plan(5L, EnginePlanStatus.ENTRY_WINDOW, List.of(futureAttempt(1, 1_000)))));
    ...
    void targetedEntryFetchesOnlyRequestedTradeAndRecordsLifecycle() {
        ...
        when(executionPort.submitOrder(any(EngineExecutionPlan.class), any(OrderIntent.class), eq(false)))
                .thenReturn(filledAttempt(5L, "bybit", "REQ/USDT", NOW));
        ArgumentCaptor<EngineTradeStateUpdateRequest> stateCaptor =
                ArgumentCaptor.forClass(EngineTradeStateUpdateRequest.class);
    ...
    void targetedExitFetchesOnlyRequestedTradeAndRecordsOutcome() {
        ...
        ArgumentCaptor<EngineTradeOutcomeRecordRequest> outcomeCaptor =
                ArgumentCaptor.forClass(EngineTradeOutcomeRecordRequest.class);
    ...
    void recordsDeterministicAttemptKeysPersistsExecutionResultAndMeasuresSubmitDuration() {
        EngineEntryAttemptPlan attemptPlan = new EngineEntryAttemptPlan(
                2, Instant.parse("2029-12-31T23:59:00.150Z"), NOW.minusSeconds(2), 0L, 150L, 0L);
    ...
    void filledEntryRecordsOpenPositionAndMovesTradeOpen() {
        when(client.listPlans(false))
                .thenReturn(List.of(plan(5L, EnginePlanStatus.ENTRY_WINDOW, List.of(pastAttempt(1, 0)))));
        when(executionPort.submitOrder(any(EngineExecutionPlan.class), any(OrderIntent.class), eq(false)))
                .thenReturn(filledAttempt(5L, "bybit", "REQ/USDT", NOW));

        var response = service.runOnce(false);
        ArgumentCaptor<EnginePositionRecordRequest> positionCaptor =
                ArgumentCaptor.forClass(EnginePositionRecordRequest.class);
        ArgumentCaptor<EngineTradeStateUpdateRequest> stateCaptor =
                ArgumentCaptor.forClass(EngineTradeStateUpdateRequest.class);
    ...
    void acknowledgedEntryDoesNotChangePositionOrTradeState() {
        ...
        assertThat(response.results())
                .singleElement()
                .satisfies(result -> assertThat(result.status()).isEqualTo(OrderAttemptStatus.ACKNOWLEDGED));
    ...
    void entryAttemptRecordsRequestDurationMs() {
        ...
        when(executionPort.submitOrder(any(EngineExecutionPlan.class), any(OrderIntent.class), eq(false)))
                .thenReturn(filledAttempt(5L, "bybit", "REQ/USDT", NOW));
        ArgumentCaptor<EngineOrderAttemptRecordRequest> attemptCaptor =
                ArgumentCaptor.forClass(EngineOrderAttemptRecordRequest.class);
    ...
    void exitWindowSubmitsReduceOnlyExitAndRecordsOutcome() {
        ...
        assertThat(response.results())
                .singleElement()
                .satisfies(result -> assertThat(result.status()).isEqualTo(OrderAttemptStatus.FILLED));
    ...
    void isolatesEntryFailuresAndContinuesProcessingOtherTrades() {
        ...
        assertThat(response.results())
                .extracting(EngineExecutionAttemptResult::armedTradeId)
    ...
    void rejectedExitMarksOnlyThatTradeFailedWithoutOutcome() {
        ...
        assertThat(response.results())
                .singleElement()
                .satisfies(result -> assertThat(result.status()).isEqualTo(OrderAttemptStatus.REJECTED));
        verify(client).updateTradeState(eq(5L), stateCaptor.capture());
        verify(client, never()).recordTradeOutcome(any());
    ...
    void longExitRecordsPositiveGrossPnlAndZeroFeesWhenFeeIsMissing() {
        when(client.listPlans(false))
                .thenReturn(List.of(openPlan(5L, TradeSide.LONG, BigDecimal.TEN, BigDecimal.valueOf(2.5))));
        when(executionPort.submitOrder(any(EngineExecutionPlan.class), any(OrderIntent.class), eq(true)))
    ...
    void stopLossTriggersEarlyExitFromWaitingExit() {
        ...
        when(client.fetchMarkPrice("bybit", "REQUSDT"))
                .thenReturn(Optional.of(new MarkPriceResponse("bybit", "REQUSDT", BigDecimal.valueOf(100), NOW)));
    ...
    void takeProfitTriggersEarlyExitFromWaitingExit() {
        ...
        when(client.listPlans(false)).thenReturn(List.of(waitingPlan));
        when(client.fetchMarkPrice("bybit", "REQUSDT"))
                .thenReturn(Optional.of(new MarkPriceResponse("bybit", "REQUSDT", BigDecimal.ONE, NOW)));
    ...
    void noEarlyExitWhenMarkPriceDoesNotBreachThreshold() {
        ...
        when(client.listPlans(false)).thenReturn(List.of(waitingPlan));
        when(client.fetchMarkPrice("bybit", "REQUSDT"))
                .thenReturn(Optional.of(new MarkPriceResponse("bybit", "REQUSDT", BigDecimal.valueOf(2.4), NOW)));
    ...
    void stopLossExactlyAtThresholdDoesNotTriggerEarlyExit() {
        ...
        when(client.listPlans(false)).thenReturn(List.of(waitingPlan));
        when(client.fetchMarkPrice("bybit", "REQUSDT"))
                .thenReturn(Optional.of(new MarkPriceResponse("bybit", "REQUSDT", BigDecimal.valueOf(2.6), NOW)));
    ...
    void noEarlyExitWhenMarkPriceNullInResponse() {
        ...
        when(client.listPlans(false)).thenReturn(List.of(waitingPlan));
        when(client.fetchMarkPrice("bybit", "REQUSDT"))
                .thenReturn(Optional.of(new MarkPriceResponse("bybit", "REQUSDT", null, NOW)));
    ...
    private static EngineExecutionPlan openPlanWithSlTp(
            Long armedTradeId, BigDecimal stopLossUsd, BigDecimal takeProfitUsd) {
    ...
    private static EngineExecutionPlan plan(
            Long armedTradeId, EnginePlanStatus status, List<EngineEntryAttemptPlan> attempts) {
        return new EngineExecutionPlan(
                armedTradeId,
                13L,
                "bybit",
    ...
    private static EngineExecutionPlan openPlan(
            Long armedTradeId, TradeSide side, BigDecimal quantity, BigDecimal entryPrice) {
        return openPlan(armedTradeId, side, quantity, entryPrice, EnginePlanStatus.EXIT_WINDOW);
    }

    private static EngineExecutionPlan openPlan(
            Long armedTradeId, TradeSide side, BigDecimal quantity, BigDecimal entryPrice, EnginePlanStatus status) {
        return new EngineExecutionPlan(
                armedTradeId,
                13L,
                "bybit",
    ...
    private static OrderAttempt filledAttemptWithoutExchangeTimestamp(
            Long armedTradeId, String venue, String symbol, Instant submittedAt) {
        return filledAttempt(
                armedTradeId, venue, symbol, TradeSide.SHORT, submittedAt, BigDecimal.valueOf(2.5), null, null, null);
    }

    private static OrderAttempt acknowledgedAttempt(
            Long armedTradeId, String venue, String symbol, Instant submittedAt) {
        return new OrderAttempt(
                null,
                null,
    ...
    private static OrderAttempt acknowledgedZeroFillExitAttempt(
            Long armedTradeId, String venue, String symbol, Instant submittedAt) {
        return new OrderAttempt(
                null,
                null,
    ...
    private static OrderAttempt acknowledgedFilledExitAttempt(
            Long armedTradeId, String venue, String symbol, Instant submittedAt) {
        return new OrderAttempt(
                null,
                null,
