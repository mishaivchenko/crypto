...
@SpringBootTest(
        ...
        properties = {"engine.internal-token=test-internal-token"})
...
class EngineApplicationIntegrationTest {
    ...
    void runOnceRecordsFailedAttemptsWhenEngineCredentialsAreMissing() throws Exception {
        // REQ: ENG-ACC-003
        MONITOR.stubFor(
                get(urlEqualTo("/internal/v1/engine/plans?includeAll=true")).willReturn(okJson("""
                [
                  {
                    "armedTradeId":5,
                    "fundingEventId":13,
                ...
                """)));
        MONITOR.stubFor(post(urlEqualTo("/internal/v1/engine/order-attempts")).willReturn(okJson("""
                {
                  "id":101,
                ...
                """)));
        MONITOR.stubFor(post(urlEqualTo("/internal/v1/engine/trades/5/state")).willReturn(okJson("""
                {
                  "armedTradeId":5,
                  "state":"FAILED",
                  "updatedAt":"2029-12-31T23:59:00Z"
        ...
        MONITOR.verify(
                2,
                postRequestedFor(urlEqualTo("/internal/v1/engine/order-attempts"))
                        .withHeader("X-Internal-Token", equalTo("test-internal-token")));
        MONITOR.verify(
                2,
                postRequestedFor(urlEqualTo("/internal/v1/engine/trades/5/state"))
                        .withHeader("X-Internal-Token", equalTo("test-internal-token")));
    }
}
