...
class GateCredentialCheckerTest {
    ...
    @Test
    void checksFuturesAccountReadiness() throws Exception {
        server.start();
        server.stubFor(com.github.tomakehurst.wiremock.client.WireMock.get(urlPathEqualTo("/futures/usdt/accounts"))
                .willReturn(okJson("{}")));

        GateCredentialChecker checker =
                new GateCredentialChecker(HttpClient.newHttpClient(), new VenueHttpProperties());
