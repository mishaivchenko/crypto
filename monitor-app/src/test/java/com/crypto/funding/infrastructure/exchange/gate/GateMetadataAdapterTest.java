...
class GateMetadataAdapterTest {
    ...
    @Test
    void derivesContractStepAndMinimumNotional() throws Exception {
        server.start();
        server.stubFor(get(urlPathEqualTo("/futures/usdt/contracts")).willReturn(okJson("""
                [
                  {
                    "name": "BTC_USDT",
                    "in_delisting": false,
        ...
        GateMetadataAdapter adapter = new GateMetadataAdapter(
                HttpClient.newHttpClient(), new VenueHttpProperties(), server.baseUrl(), venueProfileService());
        ...
        return new VenueProfileService(List.of(), null, null) {
            ...
            public ResolvedCredentials resolveCredentials(String rawVenue) {
                return new ResolvedCredentials(
                        "gate", VenueAccessMode.TESTNET, server.baseUrl(), "api-key", "secret-key", null);
            }
        };
    }
}
