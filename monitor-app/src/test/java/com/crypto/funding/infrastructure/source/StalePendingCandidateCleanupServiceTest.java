...
class StalePendingCandidateCleanupServiceTest {
    ...
    void deletesOnlyPendingCandidatesMissingFromActiveSnapshot() {
        ...
        StalePendingCandidateCleanupService cleanupService =
                new StalePendingCandidateCleanupService(repository, properties);
        ...
        when(repository.findAllBySourceTypeAndSourceChatIdAndFundingEventIdIsNullOrderByDetectedAtDesc(
                        "FUNDING_API", 1L))
    ...
    private static SignalCandidateEntity candidate(
            Long sourceMessageId, SignalCandidateStatus status, Long fundingEventId) {
