...
class EnginePlanLookaheadFilterTest {
    ...
    void alwaysKeepsActionableAndOverduePlans() {
        assertThat(filter.shouldInclude(plan(EnginePlanStatus.ENTRY_WINDOW, null), false))
                .isTrue();
        assertThat(filter.shouldInclude(plan(EnginePlanStatus.EXIT_WINDOW, null), false))
                .isTrue();
        assertThat(filter.shouldInclude(plan(EnginePlanStatus.OVERDUE, null), false))
                .isTrue();
    }

    @Test
    void usesLookaheadWindowForWaitingPlans() {
        assertThat(filter.shouldInclude(plan(EnginePlanStatus.WAITING_ENTRY, 30_000L), false))
                .isTrue();
        assertThat(filter.shouldInclude(plan(EnginePlanStatus.WAITING_ENTRY, 7_500_000L), false))
                .isFalse();
        assertThat(filter.shouldInclude(plan(EnginePlanStatus.WAITING_ENTRY, null), false))
                .isFalse();
    }

    @Test
    void includeAllBypassesLookaheadFilter() {
        assertThat(filter.shouldInclude(plan(EnginePlanStatus.CLOSED, null), true))
                .isTrue();
    }
