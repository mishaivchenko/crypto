...
class EnginePlanStatusCalculatorTest {
    ...
    private final EnginePlanStatusCalculator calculator =
            new EnginePlanStatusCalculator(properties, new EngineEntryAttemptScheduleBuilder());
    ...
    void returnsWaitingEntryBeforeFirstTrigger() {
        ...
        assertThat(calculator.deriveStatus(trade, Instant.parse("2029-12-31T23:59:00Z")))
                .isEqualTo(EnginePlanStatus.WAITING_ENTRY);
    }

    @Test
    void returnsEntryWindowInsideEntryRange() {
        ArmedTrade trade =
                trade(ArmedTradeState.ENTRY_PENDING, "2030-01-01T00:00:00Z", "2030-01-01T00:05:00Z", 3, 150L, 40L);

        assertThat(calculator.deriveStatus(trade, Instant.parse("2030-01-01T00:00:00Z")))
                .isEqualTo(EnginePlanStatus.ENTRY_WINDOW);
    }

    @Test
    void returnsOverdueAfterGraceWindow() {
        ArmedTrade trade =
                trade(ArmedTradeState.ENTRY_ATTEMPTED, "2030-01-01T00:00:00Z", "2030-01-01T00:05:00Z", 3, 150L, 40L);

        assertThat(calculator.deriveStatus(trade, Instant.parse("2030-01-01T00:00:31Z")))
                .isEqualTo(EnginePlanStatus.OVERDUE);
    }
    ...
    void returnsWaitingExitBeforeExitTime() {
        ...
        assertThat(calculator.deriveStatus(trade, Instant.parse("2030-01-01T00:04:59Z")))
                .isEqualTo(EnginePlanStatus.WAITING_EXIT);
    }

    @Test
    void returnsExitWindowAtOrAfterExitTime() {
        ArmedTrade trade =
                trade(ArmedTradeState.EXIT_PENDING, "2030-01-01T00:00:00Z", "2030-01-01T00:05:00Z", 1, 0L, 0L);

        assertThat(calculator.deriveStatus(trade, Instant.parse("2030-01-01T00:05:00Z")))
                .isEqualTo(EnginePlanStatus.EXIT_WINDOW);
    }
    ...
    void returnsClosedAndInvalidStates() {
        ...
        assertThat(calculator.deriveStatus(closed, Instant.parse("2030-01-01T00:00:00Z")))
                .isEqualTo(EnginePlanStatus.CLOSED);
        assertThat(calculator.deriveStatus(failed, Instant.parse("2030-01-01T00:00:00Z")))
                .isEqualTo(EnginePlanStatus.INVALID);
        assertThat(calculator.deriveStatus(missingEntry, Instant.parse("2030-01-01T00:00:00Z")))
                .isEqualTo(EnginePlanStatus.INVALID);
    }
