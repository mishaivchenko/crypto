...
class EnginePlanSummaryFormatterTest {
    ...
    void formatsCurrentOperatorFacingStatusText() {
        ...
        assertThat(formatter.format(trade, event, EnginePlanStatus.WAITING_ENTRY))
                .isEqualTo("Ожидаем вход по WET/USDT на gate");
        assertThat(formatter.format(trade, event, EnginePlanStatus.ENTRY_WINDOW))
                .isEqualTo("Окно входа активно для WET/USDT (SHORT)");
        assertThat(formatter.format(trade, event, EnginePlanStatus.WAITING_EXIT))
                .isEqualTo("Сделка ждёт планового выхода");
        assertThat(formatter.format(trade, event, EnginePlanStatus.EXIT_WINDOW))
                .isEqualTo("Окно выхода активно для WET/USDT");
        assertThat(formatter.format(trade, event, EnginePlanStatus.OVERDUE))
                .isEqualTo("Окно входа пропущено, нужен разбор оператора");
        assertThat(formatter.format(trade, event, EnginePlanStatus.CLOSED)).isEqualTo("Сделка уже закрыта");
        assertThat(formatter.format(trade, event, EnginePlanStatus.INVALID))
                .isEqualTo("План сделки неполный и не готов к исполнению");
    }
}
