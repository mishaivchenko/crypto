...
class MonitorRuntimeProfilesIntegrationTest {
    ...
    void localSafeProfileDisablesOperatorAuthAndCredentialStorage() {
        try (ConfigurableApplicationContext context =
                runWithProfile("local-safe", tempDir.resolve("local-safe.sqlite"))) {
            assertThat(context.getBean(OperatorSecurityProperties.class).isAuthEnabled())
                    .isFalse();
            assertThat(context.getBean(CredentialStorageProperties.class).isEnabled())
                    .isFalse();
            assertThat(context.getBean(MetadataSyncProperties.class).isRequireCredentialsOnStartup())
                    .isFalse();
            assertThat(context.getBean(MonitorEngineMetricsProperties.class).isEnabled())
                    .isFalse();
            assertThat(context.getEnvironment().getProperty("spring.jpa.open-in-view", Boolean.class))
                    .isFalse();
        }
    }
    ...
    void prodLikeProfileEnablesAuthAndCredentialStorageAndRequiresMetadataCredentials() {
        try (ConfigurableApplicationContext context =
                runWithProfile("prod-like", tempDir.resolve("prod-like.sqlite"))) {
            assertThat(context.getBean(OperatorSecurityProperties.class).isAuthEnabled())
                    .isTrue();
            assertThat(context.getBean(CredentialStorageProperties.class).isEnabled())
                    .isTrue();
