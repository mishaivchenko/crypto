...
class OperatorSecurityIntegrationTest {
    ...
    void storesEncryptedCredentialsPerOperator() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/operators/me/credentials").header("X-Operator-Token", "bob-token"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$").isEmpty());

        mockMvc.perform(get("/api/v1/operators/me/credentials").header("X-Operator-Token", "alice-token"))
                .andExpect(status().isOk())
    ...
    void checksStoredCredentialsWithoutReturningSecrets() throws Exception {
        BYBIT_SERVER.stubFor(
                com.github.tomakehurst.wiremock.client.WireMock.get(urlPathEqualTo("/v5/account/wallet-balance"))
                        .willReturn(okJson("""
                {
                  "retCode": 10003,
