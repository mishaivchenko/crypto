...
class VenueDiagnosticsApiIntegrationTest {
    private static final WireMockServer BYBIT_SERVER =
    ...
    void clean() {
        ...
        BYBIT_SERVER.stubFor(get(urlPathEqualTo("/v5/market/instruments-info")).willReturn(okJson("""
                                       {
                                         "retCode": 0,
                                         "retMsg": "OK",
                                         "result": {
    ...
    void syncsVenueMetadataAndExposesDiagnostics() throws Exception {
        mockMvc.perform(post("/api/v1/venues/bybit/sync").contentType(MediaType.APPLICATION_JSON))
        ...
        mockMvc.perform(org.springframework.test.web.servlet.request.MockMvcRequestBuilders.get(
                        "/api/v1/venues/bybit/instruments"))
                .andExpect(status().isOk())
    ...
    void usesSyncedRegistryForCandidateNormalization() throws Exception {
        ...
        mockMvc.perform(org.springframework.test.web.servlet.request.MockMvcRequestBuilders.get(
                        "/api/v1/candidates/{id}", candidateId))
                .andExpect(status().isOk())
    ...
    void allowsRuntimeModeSwitchAndReportsMissingCredentials() throws Exception {
        ...
        BYBIT_SERVER.stubFor(get(urlPathEqualTo("/v5/account/wallet-balance")).willReturn(okJson("""
                                       {
                                         "retCode": 0,
                                         "retMsg": "OK",
                                         "result": {}
