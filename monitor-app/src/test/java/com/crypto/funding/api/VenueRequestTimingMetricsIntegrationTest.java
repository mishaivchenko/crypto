...
class VenueRequestTimingMetricsIntegrationTest {
    ...
    void exportsVenueRequestTimingMetrics() {
        ...
        assertThat(prometheus)
                .contains("funding_venue_request_avg_duration_ms{operation=\"metadata-sync\",venue=\"gate\"} 42.0");
        assertThat(prometheus)
                .contains("funding_venue_request_last_duration_ms{operation=\"credential-check\",venue=\"gate\"} 11.0");
        assertThat(prometheus).contains("funding_venue_requests_total{operation=\"metadata-sync\",venue=\"gate\"} 1.0");
        assertThat(prometheus)
                .contains("funding_venue_request_failures_total{operation=\"credential-check\",venue=\"gate\"} 1.0");
        assertThat(prometheus)
                .contains("funding_venue_request_avg_duration_ms{operation=\"metadata-sync\",venue=\"bybit\"} 0.0");
    }
