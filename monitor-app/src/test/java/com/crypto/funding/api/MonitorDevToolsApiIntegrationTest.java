...
class MonitorDevToolsApiIntegrationTest {
    ...
    void resetEngine() {
        ...
        ENGINE.stubFor(com.github.tomakehurst.wiremock.client.WireMock.get(urlPathEqualTo("/internal/engine/runtime"))
                ...
                """)));
        ENGINE.stubFor(com.github.tomakehurst.wiremock.client.WireMock.post(
                        urlPathEqualTo("/internal/engine/execution/target"))
                .willReturn(okJson("""
                {
                ...
                """)));
        ENGINE.stubFor(com.github.tomakehurst.wiremock.client.WireMock.post(
                        urlPathEqualTo("/internal/engine/execution/run-once"))
    ...
    void loadsAndUpdatesEngineRuntimeThroughMonitorApi() throws Exception {
        // REQ: ENG-ACC-007
        mockMvc.perform(get("/api/v2/monitor/dev/engine/runtime").header("X-Operator-Token", "alice-token"))
                .andExpect(status().isOk())
        ...
        ENGINE.verify(com.github.tomakehurst.wiremock.client.WireMock.getRequestedFor(
                        urlPathEqualTo("/internal/engine/runtime"))
                .withHeader("X-Internal-Token", equalTo("test-internal-token")));
    ...
    void listsDevTestRunOptionsForActiveBybitAndGateSymbols() throws Exception {
        ...
        mockMvc.perform(get("/api/v2/monitor/dev/test-runs/options").header("X-Operator-Token", "alice-token"))
                .andExpect(status().isOk())
    ...
    void productionTargetExecutionRequiresTypedConfirmation() throws Exception {
        ...
        mockMvc.perform(post("/api/v2/monitor/dev/test-runs/{armedTradeId}/entry", armedTradeId)
                ...
                .andExpect(jsonPath("$.message")
                        .value("Production dev test run requires confirmation: bybit BTC/USDT LIVE"));
    }
    ...
    void productionTargetExecutionRejectsWhenEngineLiveGatesAreNotAligned() throws Exception {
        ...
        mockMvc.perform(post("/api/v2/monitor/dev/test-runs/{armedTradeId}/entry", armedTradeId)
                ...
                .andExpect(status().isConflict())
                .andExpect(jsonPath("$.message")
                        .value("Production dev test run safety gate failed: ENGINE_LIVE_ORDER_ENABLED is false."));
    }
    ...
    private Long createDevRun(String venue, String symbol) throws Exception {
        mockMvc.perform(post("/api/v2/monitor/dev/test-runs")
                ...
                .andExpect(status().isCreated());
        return armedTradeRepository.findAll().stream()
