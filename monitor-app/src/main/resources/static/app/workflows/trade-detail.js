...
import { renderLayerPipeline, getLayerCollapsed } from "../components/layer-pipeline.js";
...
function buildLiquidityLayerDescriptor(liquidity, trade) {
    ...
    const layerStatus = liquidity.score === "UNTRADABLE" ? "blocked"
        : liquidity.score === "THIN" ? "warn"
        : liquidity.score === "EXCELLENT" || liquidity.score === "GOOD" ? "ok"
        : "warn";
    ...
    return {
        layerType: "liquidity",
        layerName: t("layer.liquidity"),
        decoratorName: escapeHtml(trade.venue ?? ""),
        timestamp: liquidity.sampledAt ?? null,
        source: escapeHtml(effectiveSymbol ?? ""),
        status: layerStatus,
        collapsed: getLayerCollapsed("trade", "liquidity", false),
        content: `
            ${warning}
...
export function buildLatencyChainSection(trade) {
    ...
    return {
        ...
        decoratorName: trade.armSource ? escapeHtml(trade.armSource) : "",
        timestamp: trade.armedAt ?? null,
        source: "",
        status: "ok",
        collapsed: getLayerCollapsed("trade", "latency", false),
        content: `
...
export function buildTradeExpansionContent({ trade, attempts = [], liquidity = null, position = null, outcome = null }) {
    const layers = [
        buildLiquidityLayerDescriptor(liquidity, trade),
        buildLatencyChainSection(trade),
    ].filter(Boolean);
    return `
        ${renderLayerPipeline(layers, { screen: "trade" })}
        ...
        ${trade.state === "ARMED" ? `
            <details class="technical-details">
                ...
                <form class="compact-form" data-action="edit-trade" data-id="${trade.id}">
                  ...
                  <details>
                    <summary>${t("trade_note") || "Note"} (${t("label_optional") || "optional"})</summary>
