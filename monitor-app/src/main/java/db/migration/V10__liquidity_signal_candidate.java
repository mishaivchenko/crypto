...
public class V10__liquidity_signal_candidate extends BaseJavaMigration {
    ...
    public void migrate(Context context) throws Exception {
        ...
        if (!indexExists(connection)) {
            try (Statement statement = connection.createStatement()) {
                statement.execute(
                        "CREATE INDEX idx_liquidity_assessment_signal_candidate_id ON liquidity_assessment (signal_candidate_id)");
            }
        }
    }
    ...
    private static boolean indexExists(Connection connection) throws Exception {
        try (Statement statement = connection.createStatement();
                ResultSet rs = statement.executeQuery(
                        "SELECT name FROM sqlite_master WHERE type='index' AND name='idx_liquidity_assessment_signal_candidate_id'")) {
            return rs.next();
        }
    }
}
