...
@ConditionalOnProperty(
        prefix = "trading.candidate-source",
        name = "enabled",
        havingValue = "true",
        matchIfMissing = true)
        ...
        try {
            ...
            for (FundingApiEntry entry : response.data()) {
                ...
                if (resolvedSymbol.isEmpty()) {
                    log.debug(
                            "[candidate-source] skip unresolved entry exchange={} symbol={} coin={}",
                            entry.exchange(),
                            entry.symbol(),
                            entry.coin());
                    continue;
                ...
                FundingObservation observation = observationMapper.toObservation(
                        entry, resolvedSymbol.get().canonicalSymbol());
            ...
            venueRequestTimingService.recordSuccess(
                    ...
                    200);
            log.info(
                    "[candidate-source] refreshed candidates from funding API entries={} processed={}",
                    response.data().size(),
                    processed);
        } catch (Exception ex) {
            venueRequestTimingService.recordFailure(
                    TIMING_VENUE, TIMING_OPERATION, System.nanoTime() - startNanos, ex.getMessage());
