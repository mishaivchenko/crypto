...
public interface LiquidityAssessmentJpaRepository extends JpaRepository<LiquidityAssessmentEntity, Long> {
     ...
     */
    @Query("SELECT la.venue, COUNT(DISTINCT la.tradeId) " + "FROM LiquidityAssessmentEntity la "
            ...
            + "  ) "
            + "GROUP BY la.venue")
    List<Object[]> countEnrichedArmedTradesByVenue(
            @Param("states") Set<com.crypto.funding.domain.trade.ArmedTradeState> states);
}
