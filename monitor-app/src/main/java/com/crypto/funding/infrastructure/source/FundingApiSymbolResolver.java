...
class FundingApiSymbolResolver {
    ...
    Optional<ResolvedFundingSymbol> resolve(FundingApiEntry entry) {
        ...
        for (String candidate : candidates) {
            Optional<String> canonicalByVenueSymbol =
                    symbolMetadataPort.findByVenueSymbol(venue, candidate).map(symbol -> symbol.symbol());
        ...
        for (String candidate : candidates) {
            ...
            if (unified != null
                    && symbolMetadataPort.findSymbolMetadata(venue, unified).isPresent()) {
