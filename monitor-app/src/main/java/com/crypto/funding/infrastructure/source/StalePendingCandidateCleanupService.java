...
class StalePendingCandidateCleanupService {
    ...
    StalePendingCandidateCleanupService(
            SignalCandidateJpaRepository signalCandidateRepository, FundingCandidateSourceProperties sourceProperties) {
    ...
    void deleteMissingPendingCandidates(Set<Long> activeSourceMessageIds) {
        ...
        List<SignalCandidateEntity> staleCandidates = signalCandidateRepository
                .findAllBySourceTypeAndSourceChatIdAndFundingEventIdIsNullOrderByDetectedAtDesc(
                        sourceType, SOURCE_STREAM_ID)
                .stream()
