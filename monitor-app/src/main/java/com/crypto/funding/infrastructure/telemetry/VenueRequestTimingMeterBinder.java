...
public class VenueRequestTimingMeterBinder implements MeterBinder {
    private static final List<String> OPERATIONS =
            List.of("metadata-sync", "credential-check", "uainvest-funding-fetch");
    ...
    public VenueRequestTimingMeterBinder(
            VenueRequestTimingService timingService, MetadataSyncProperties metadataSyncProperties) {
    ...
    public void bindTo(MeterRegistry registry) {
        for (TimingKey key : timingKeys) {
            FunctionCounter.builder(
                            "funding_venue_requests",
                            key,
                            holder -> snapshot(holder).requests())
                    .description("Total venue-facing requests captured by monitor telemetry.")
                    .tags("venue", key.venue(), "operation", key.operation())
                    .register(registry);

            FunctionCounter.builder(
                            "funding_venue_request_successes",
                            key,
                            holder -> snapshot(holder).successes())
                    .description("Successful venue-facing requests captured by monitor telemetry.")
                    .tags("venue", key.venue(), "operation", key.operation())
                    .register(registry);

            FunctionCounter.builder(
                            "funding_venue_request_failures",
                            key,
                            holder -> snapshot(holder).failures())
                    .description("Failed venue-facing requests captured by monitor telemetry.")
                    .tags("venue", key.venue(), "operation", key.operation())
                    .register(registry);

            Gauge.builder(
                            "funding_venue_request_avg_duration_ms",
                            key,
                            holder -> snapshot(holder).averageDurationMs())
                    .description("Average duration of venue-facing requests in milliseconds.")
                    .tags("venue", key.venue(), "operation", key.operation())
                    .register(registry);

            Gauge.builder(
                            "funding_venue_request_last_duration_ms",
                            key,
                            holder -> snapshot(holder).lastDurationMs())
            ...
            Gauge.builder(
                            "funding_venue_request_last_payload_size",
                            key,
                            holder -> snapshot(holder).lastPayloadSize())
