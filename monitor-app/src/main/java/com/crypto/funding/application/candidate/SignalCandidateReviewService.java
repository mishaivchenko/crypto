...
public class SignalCandidateReviewService {
    ...
    public SignalCandidate approve(ApproveSignalCandidateCommand command) {
        ...
        Instant fundingTime = command.fundingTime() != null ? command.fundingTime() : entity.getSourceFundingTime();
        BigDecimal fundingRatePct =
                command.fundingRatePct() != null ? command.fundingRatePct() : entity.getSourceFundingRatePct();
    ...
    public SignalCandidate reject(RejectSignalCandidateCommand command) {
        ...
        entity.setReviewNotes(
                command.reviewNotes() != null ? command.reviewNotes().trim() : null);
    ...
    private void validateReviewable(SignalCandidateEntity entity) {
        if (entity.getStatus() == SignalCandidateStatus.REJECTED
            ...
            throw new DomainValidationException(
                    "Кандидат " + entity.getId() + " уже находится в терминальном статусе " + entity.getStatus());
        }
    }
    ...
    private String resolveVenue(SignalCandidateEntity entity, String overrideVenue) {
        ...
        if (venueHints.size() != 1) {
            throw new DomainValidationException(
                    "Нужно явно указать площадку, потому что кандидат не резолвится в одну venue.");
        }
