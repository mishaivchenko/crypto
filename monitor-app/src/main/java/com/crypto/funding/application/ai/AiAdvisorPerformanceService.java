...
public class AiAdvisorPerformanceService {
    ...
    public record RecommendationStat(String recommendation, int tradeCount, Double winRate, BigDecimal avgPnlUsd) {}

    public record PerformanceStats(List<RecommendationStat> stats, int totalTrades) {}

    @Transactional(readOnly = true)
    public PerformanceStats getPerformanceStats() {
        List<Object[]> rows = adviceRepository.findPerformanceStats();

        Map<String, Object[]> byRec = rows.stream().collect(Collectors.toMap(r -> String.valueOf(r[0]), r -> r));
        ...
        List<RecommendationStat> stats = Arrays.stream(AiRecommendation.values())
                .map(rec -> {
                    ...
                    BigDecimal avgPnl =
                            row[3] != null ? new BigDecimal(row[3].toString()).setScale(2, RoundingMode.HALF_UP) : null;
                    return new RecommendationStat(rec.name(), tradeCount, winRate, avgPnl);
                })
                .toList();

        int totalTrades =
                stats.stream().mapToInt(RecommendationStat::tradeCount).sum();
