...
public class EngineLifecycleRecordService {
    ...
    public EngineTradeStateUpdateResponse updateTradeState(Long armedTradeId, EngineTradeStateUpdateRequest request) {
        ...
        ArmedTradeEntity trade = armedTradeRepository
                ...
                .orElseThrow(() -> new ResourceNotFoundException("Prepared trade not found: " + armedTradeId));
    ...
    public EnginePositionResponse recordPosition(EnginePositionRecordRequest request) {
        ...
        ArmedTradeEntity armedTrade = armedTradeRepository
                .findById(request.armedTradeId())
                .orElseThrow(
                        () -> new ResourceNotFoundException("Prepared trade not found: " + request.armedTradeId()));
        PositionEntity entity = positionRepository
        ...
        if (request.state() == PositionState.CLOSED) {
            ...
            if (armedTrade.getNotes() == null
                    || armedTrade.getNotes().isBlank()
                    || "entry filled".equalsIgnoreCase(armedTrade.getNotes())) {
