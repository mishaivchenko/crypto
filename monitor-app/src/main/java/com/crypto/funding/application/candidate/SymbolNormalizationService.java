...
public class SymbolNormalizationService {
    ...
    public SymbolNormalizationService(
            SymbolMetadataPort symbolMetadataPort, MetadataSyncProperties metadataSyncProperties) {
    ...
    public CandidateNormalizationResult normalize(String rawSymbol, String sourceVenue) {
        ...
        if (normalized == null || !CANONICAL_SYMBOL.matcher(normalized).matches()) {
            return new CandidateNormalizationResult(
                    null, List.of(), "Не удалось привести символ к каноническому виду perpetual-контракта.");
        }
        ...
        if (lockedVenue != null) {
            if (symbolMetadataPort.findByVenueSymbol(lockedVenue, rawSymbol).isPresent()
                    || symbolMetadataPort
                            .findSymbolMetadata(lockedVenue, normalized)
                            .isPresent()
        ...
        List<String> venueHints = metadataSyncProperties.getEnabledVenues().stream()
                .filter(Objects::nonNull)
                .map(value -> value.trim().toLowerCase(Locale.ROOT))
                .filter(venue -> !venue.isBlank())
                .filter(venue ->
                        symbolMetadataPort.findSymbolMetadata(venue, normalized).isPresent())
                .distinct()
                .sorted(Comparator.naturalOrder())
                .toList();

        if (venueHints.isEmpty() && metadataSyncProperties.isBootstrapFallbackEnabled()) {
            venueHints = metadataSyncProperties.getEnabledVenues().stream()
                    .filter(Objects::nonNull)
                    .map(value -> value.trim().toLowerCase(Locale.ROOT))
                    .filter(venue -> !venue.isBlank())
                    .distinct()
        ...
        if (venueHints.isEmpty()) {
            return new CandidateNormalizationResult(
                    normalized, List.of(), "Для нормализованного символа не найдено поддерживаемых площадок.");
        }
    ...
    private String normalizeVenue(String rawVenue) {
        ...
        boolean enabled = metadataSyncProperties.getEnabledVenues().stream()
                .filter(Objects::nonNull)
                .map(value -> value.trim().toLowerCase(Locale.ROOT))
