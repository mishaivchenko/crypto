...
public class ArmedTradeCommandService {
    ...
    private static final Set<ArmedTradeState> CANCELLABLE_STATES =
            Set.of(ArmedTradeState.ARMED, ArmedTradeState.ENTRY_PENDING, ArmedTradeState.ENTRY_ATTEMPTED);

    @Transactional
    public ArmedTrade cancel(Long id) {
        ArmedTradeEntity entity = armedTradeRepository
                .findById(id)
    ...
    public ArmedTrade update(UpdateArmedTradeCommand command) {
        ...
        if (entity.getState() != ArmedTradeState.ARMED) {
            throw new DomainValidationException(
                    "Редактирование возможно только в статусе ARMED. Текущий статус: " + entity.getState());
        }
        ...
        if (command.plannedEntryAt() != null
                && command.plannedExitAt() != null
                && command.plannedExitAt().isBefore(command.plannedEntryAt())) {
        ...
        entity.setEffectiveEntryLatencyMs(venueLatencyService.effectiveEntryLatencyMs(
                entity.getMeasuredEntryLatencyMs(), manualLatencyAdjustmentMs));

        FundingEventEntity fundingEvent = fundingEventRepository
                .findById(entity.getFundingEventId())
                .orElseThrow(() ->
                        new ResourceNotFoundException("Событие фандинга не найдено: " + entity.getFundingEventId()));
        entity.setEntryLeadMs(
                command.plannedEntryAt() == null
                        ? null
                        : Duration.between(command.plannedEntryAt(), fundingEvent.getFundingTime())
                                .toMillis());
        entity.setExitLeadMs(
                command.plannedExitAt() == null
                        ? null
                        : Duration.between(command.plannedExitAt(), fundingEvent.getFundingTime())
                                .toMillis());
    ...
    public ArmedTrade create(
            ...
            String actorRef) {
        FundingEventEntity fundingEvent = fundingEventRepository
                .findById(command.fundingEventId())
                .orElseThrow(() ->
                        new ResourceNotFoundException("Событие фандинга не найдено: " + command.fundingEventId()));
        ...
        if (fundingEvent.getStatus() == FundingEventStatus.CANCELLED
            ...
            throw new DomainValidationException("Событие фандинга " + command.fundingEventId()
                    + " нельзя подготовить из статуса " + fundingEvent.getStatus());
        }
        if (armedTradeRepository.existsByFundingEventIdAndStateIn(fundingEvent.getId(), ACTIVE_STATES)) {
            throw new DomainValidationException(
                    "У события фандинга " + command.fundingEventId() + " уже есть активная подготовленная сделка.");
        }
        String normalizedVenue = fundingEvent.getVenue() == null
                ? ""
        ...
        entity.setEntryAttemptCount(
                command.entryAttemptCount() == null
                        ? preparationProperties.getDefaultEntryAttemptCount()
                        : command.entryAttemptCount());
        entity.setEntrySpacingMs(
                command.entrySpacingMs() == null
                        ? preparationProperties.getDefaultEntrySpacingMs()
                        : command.entrySpacingMs());
        Long venueDefaultLatencyMs =
                venueProfileService.getProfile(normalizedVenue).defaultManualLatencyAdjustmentMs();
        Long manualLatencyAdjustmentMs = command.manualLatencyAdjustmentMs() != null
                ? command.manualLatencyAdjustmentMs()
                : (venueDefaultLatencyMs != null
                        ? venueDefaultLatencyMs
        ...
        entity.setEffectiveEntryLatencyMs(
                venueLatencyService.effectiveEntryLatencyMs(measuredEntryLatencyMs, manualLatencyAdjustmentMs));
        Instant armedAt = Instant.now();
        entity.setArmedAt(armedAt);
        entity.setEventAgeMsAtArm(
                Duration.between(fundingEvent.getDiscoveredAt(), armedAt).toMillis());
        entity.setEntryLeadMs(
                command.plannedEntryAt() == null
                        ? null
                        : Duration.between(command.plannedEntryAt(), fundingEvent.getFundingTime())
