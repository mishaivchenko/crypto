...
public class AiSignalAdvisorService {
    ...
    public AiSignalAdvice analyze(Long candidateId) {
        ...
        VenueRequestTimingService.Snapshot latency =
                venue != null ? venueRequestTimingService.snapshot(venue, "order_book") : null;
        ...
        log.info(
                "AI advice for candidate {}: {} (confidence={:.2f})",
                candidateId,
    ...
    public Optional<AiSignalAdvice> findLatest(Long candidateId) {
        return adviceRepository
                .findFirstBySignalCandidateIdOrderByAnalyzedAtDesc(candidateId)
                .map(this::toDomain);
    }
    ...
    private String buildPrompt(
        ...
        sb.append("Биржа: ")
                .append(resolveVenue(candidate) != null ? resolveVenue(candidate) : "неизвестно")
                .append("\n");

        if (candidate.sourceFundingRatePct() != null) {
            sb.append("Ставка финансирования: ")
                    .append(candidate.sourceFundingRatePct())
                    .append("%");
        ...
        if (liquidity != null) {
            ...
            if (liquidity.roundTripSafeNotional() != null) {
                sb.append(", безопасный объём: $")
                        .append(liquidity.roundTripSafeNotional().setScale(0, java.math.RoundingMode.HALF_UP));
            }
    ...
    private void appendTimingSection(
            StringBuilder sb, SignalCandidate candidate, VenueRequestTimingService.Snapshot latency) {
        if (candidate.sourceFundingTime() == null) {
            return;
        }
        long secondsUntilFunding =
                Duration.between(Instant.now(), candidate.sourceFundingTime()).toSeconds();
        ...
        if (secondsUntilFunding <= 60) {
            ...
            sb.append("До момента фандинга: ")
                    .append(secondsUntilFunding)
                    .append(" сек  ← ")
                    .append(urgency)
                    .append("\n");
        ...
        if (latency != null
                && latency.p50DurationMs() != null
                && latency.p50DurationMs() > 0
                && secondsUntilFunding > 0
                && secondsUntilFunding <= 600) {
            ...
            if (usableWindowMs > 0) {
                ...
                sb.append("Оценочное окно исполнения: ~")
                        .append(estimatedAttempts)
                        .append(" попыток (p50=")
                        .append(latency.p50DurationMs())
    ...
    private void appendPerformanceSection(StringBuilder sb, AiAdvisorPerformanceService.PerformanceStats perfStats) {
        ...
        sb.append("\nИсторическая эффективность советника (последние ")
                .append(perfStats.totalTrades())
                .append(" сделок):\n");
        for (AiAdvisorPerformanceService.RecommendationStat stat : perfStats.stats()) {
            if (stat.tradeCount() == 0) {
                ...
                String winPct = stat.winRate() != null ? String.format("%.0f%%", stat.winRate() * 100) : "—";
                String avgPnl = stat.avgPnlUsd() != null
                        ? (stat.avgPnlUsd().compareTo(java.math.BigDecimal.ZERO) >= 0 ? "+" : "") + "$"
                                + stat.avgPnlUsd()
                        : "—";
                sb.append("- ")
                        .append(stat.recommendation())
                        .append(": ")
                        .append(stat.tradeCount())
                        .append(" сделок, win rate ")
                        .append(winPct)
                        .append(", avg PnL ")
                        .append(avgPnl)
                        .append("\n");
