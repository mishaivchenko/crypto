...
public class EngineMetricsSnapshotView {
    ...
    public double lastRunAgeSeconds() {
        ...
        return Math.max(
                0D,
                Duration.between(snapshot.lastRunFinishedAt(), Instant.now(clock))
                                .toMillis()
                        / 1000D);
    }
    ...
    public double lastForcedRunAgeSeconds() {
        ...
        return Math.max(
                0D,
                Duration.between(snapshot.lastForcedRunFinishedAt(), Instant.now(clock))
                                .toMillis()
                        / 1000D);
    }
    ...
    public double snapshotAgeSeconds() {
        ...
        return Math.max(
                0D, Duration.between(snapshot.capturedAt(), Instant.now(clock)).toMillis() / 1000D);
    }
