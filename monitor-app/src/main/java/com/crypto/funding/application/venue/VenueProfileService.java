...
public class VenueProfileService {
    ...
    public record GlobalAccessProfile(
            VenueAccessMode mode, boolean modeOverridden, List<VenueAccessMode> availableModes) {}
    ...
    public record ResolvedCredentials(
            String venue, VenueAccessMode mode, String baseUrl, String apiKey, String secretKey, String passphrase) {
        public boolean credentialsLoaded() {
    ...
    public VenueProfileService(
            List<VenueCredentialCheckPort> checkers, VenueProfileJpaRepository repository, Environment environment) {
        this.checkersByVenue = checkers.stream()
                .collect(Collectors.toUnmodifiableMap(
                        checker -> checker.venue().toLowerCase(Locale.ROOT), checker -> checker));
    ...
    public VenueAccessProfile getProfile(String rawVenue) {
        ...
        List<VenueAccessMode> availableModes =
                checker == null || checker.supportedModes().isEmpty()
        ...
        return new VenueAccessProfile(
                ...
                entity == null || entity.getConnectionStatus() == null
    ...
    public GlobalAccessProfile getGlobalAccessProfile() {
        ...
        VenueAccessMode mode =
                entity != null && entity.getSelectedMode() != null ? entity.getSelectedMode() : defaultGlobalMode();
    ...
    public Map<String, VenueAccessProfile> getProfiles(Set<String> venues) {
        ...
        venues.stream()
                .map(VenueProfileService::normalizeVenue)
                .sorted()
                .forEach(venue -> profiles.put(venue, getProfile(venue)));
        return profiles;
    }
    ...
    public VenueAccessProfile updateDefaultLatency(String rawVenue, Long defaultManualLatencyAdjustmentMs) {
        ...
        VenueProfileEntity entity = repository.findById(venue).orElseGet(() -> {
            ...
            created.setSelectedMode(resolveModeForVenue(
                    venue, availableModes(venue), getGlobalAccessProfile().mode()));
            return created;
        });
    ...
    private VenueAccessMode defaultMode(String venue, List<VenueAccessMode> availableModes) {
        String rawMode = environment.getProperty(
                "trading." + venue + ".mode", availableModes.getFirst().propertyValue());
    ...
    private VenueAccessMode resolveModeForVenue(
            String venue, List<VenueAccessMode> availableModes, VenueAccessMode globalMode) {
    ...
    private static boolean requiresPassphrase(String venue) {
        return "bitget".equalsIgnoreCase(venue) || "okx".equalsIgnoreCase(venue) || "kucoin".equalsIgnoreCase(venue);
    }
