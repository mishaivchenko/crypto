...
public class SignalCandidateIngestService {
    ...
    public SignalCandidate ingest(IngestSignalCandidateCommand command) {
        if (command.sourceMessageId() != null) {
            SignalCandidateEntity existing = candidateRepository
                    .findBySourceTypeAndSourceChatIdAndSourceMessageId(
            ...
            Instant threshold =
                    command.detectedAt().minus(Duration.ofMinutes(candidateProperties.getDedupeWindowMinutes()));
            SignalCandidateEntity existing = candidateRepository
                    .findFirstBySourceTypeAndRawSymbolAndDetectedAtGreaterThanEqualOrderByDetectedAtDesc(
                            normalizeSourceType(command.sourceType()),
    ...
    private SignalCandidate refreshExistingCandidate(
            SignalCandidateEntity existing, IngestSignalCandidateCommand command) {
    ...
    private void applyObservation(SignalCandidateEntity entity, IngestSignalCandidateCommand command) {
        CandidateNormalizationResult normalizationResult =
                symbolNormalizationService.normalize(command.rawSymbol(), command.sourceVenue());
        ...
        entity.setStatus(
                normalizationResult.isNormalized() ? SignalCandidateStatus.NORMALIZED : SignalCandidateStatus.FAILED);
