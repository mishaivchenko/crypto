...
public class DevTestRunService {
    ...
    public DevTestRunOptionsResponse options() {
        ...
        List<String> enabledVenues = properties.enabledVenues().stream()
        ...
        for (String venue : enabledVenues) {
            if (!credentialStatus.getOrDefault(venue, false)) {
                safetyIssues.add("Engine credentials missing for " + venue.toUpperCase(Locale.ROOT)
                        + ". Add API key and secret for " + venue
                        + " in the monitor venue settings, then reload engine credentials.");
            }
        }
        return new DevTestRunOptionsResponse(
                isToolEnabled(mode),
                mode,
                enabledVenues.stream().map(this::venueOption).toList(),
                toResponse(runtime),
                safetyIssues);
    }
    ...
    public DevTestRunExecutionResponse runPhase(
            Long armedTradeId, EngineExecutionTargetPhase phase, String productionConfirm) {
        ArmedTradeEntity trade = armedTradeRepository
                .findById(armedTradeId)
                .orElseThrow(() -> new ResourceNotFoundException("Подготовленная сделка не найдена: " + armedTradeId));
        FundingEventEntity event = fundingEventRepository
                .findById(trade.getFundingEventId())
                .orElseThrow(() ->
                        new ResourceNotFoundException("Событие фандинга не найдено: " + trade.getFundingEventId()));
        ...
        if (mode == VenueAccessMode.PRODUCTION) {
            ...
            List<String> productionIssues =
                    productionExecutionIssues(engineControlService.runtime(), event.getVenue(), trade.getNotionalUsd());
            if (!productionIssues.isEmpty()) {
                throw new DomainValidationException(
                        "Production dev test run safety gate failed: " + String.join("; ", productionIssues));
            }
        }
    ...
    private DevTestRunVenueOption venueOption(String venue) {
        List<DevTestRunSymbolOption> symbols =
                instrumentMetadataRepository.findAllByVenueOrderByCanonicalSymbolAsc(venue).stream()
                        .filter(instrument -> instrument.getStatus() == InstrumentStatus.ACTIVE)
                        .map(instrument -> new DevTestRunSymbolOption(
                                instrument.getCanonicalSymbol(), instrument.getVenueSymbol()))
                        .toList();
    ...
    private InstrumentMetadataEntity requireActiveInstrument(String venue, String symbol) {
        return instrumentMetadataRepository
                .findByVenueAndCanonicalSymbolAndStatus(venue, symbol, InstrumentStatus.ACTIVE)
                .orElseThrow(() ->
                        new DomainValidationException("Dev test run symbol is not active on " + venue + ": " + symbol));
    }
    ...
    private void validateNotional(BigDecimal notionalUsd) {
        ...
        BigDecimal maxNotionalUsd =
                properties.getMaxNotionalUsd() == null ? BigDecimal.valueOf(25) : properties.getMaxNotionalUsd();
    ...
    private void assertToolEnabled(VenueAccessMode mode) {
        if (!isToolEnabled(mode)) {
            throw new DomainValidationException(
                    "Dev test run tool is disabled for production. Set MONITOR_DEV_TEST_TOOL_ENABLED=true.");
        }
    }
    ...
    private List<String> productionExecutionIssues(
            EngineRuntimeControlResponse runtime, String venue, BigDecimal notionalUsd) {
        List<String> issues = new ArrayList<>();
        if (!runtime.liveOrderEnabled()) {
            issues.add("ENGINE_LIVE_ORDER_ENABLED is false.");
        }
        ...
        List<String> enabledVenues = runtime.liveEnabledVenues() == null
                ? List.of()
                : runtime.liveEnabledVenues().stream()
        ...
        BigDecimal maxNotionalUsd =
                runtime.maxNotionalUsd() == null ? properties.getMaxNotionalUsd() : runtime.maxNotionalUsd();
