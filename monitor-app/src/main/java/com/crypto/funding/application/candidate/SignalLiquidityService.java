...
public class SignalLiquidityService {
    ...
    public Optional<LiquidityAssessment> assess(SignalCandidate candidate) {
        ...
        String venueSymbol = instrumentRegistryService
                .resolveVenueSymbol(venue, candidate.normalizedSymbol())
                .orElse(null);
        if (venueSymbol == null) {
            log.debug(
                    "Skipping liquidity assessment for candidate {} — no instrument metadata for {}/{}",
                    candidate.id(),
                    venue,
        ...
        try {
            LiquidityAssessment assessment =
                    liquidityAssessmentService.assessForCandidate(venue, venueSymbol, candidate.id());
            log.debug(
                    "Liquidity assessment for candidate {} ({}/{}): {}",
                    candidate.id(),
                    venue,
                    venueSymbol,
                    assessment.score());
            ...
            log.warn(
                    "Liquidity assessment failed for candidate {} ({}/{}): {}",
                    candidate.id(),
                    venue,
                    venueSymbol,
                    e.getMessage());
