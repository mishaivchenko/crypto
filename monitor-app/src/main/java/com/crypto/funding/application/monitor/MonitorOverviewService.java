...
public class MonitorOverviewService {
    ...
    public Overview load() {
        ...
        long pendingCandidates = signalCandidateRepository.findAll().stream()
                .filter(candidate -> candidate.getStatus() == SignalCandidateStatus.NEW
                        || candidate.getStatus() == SignalCandidateStatus.NORMALIZED)
                .count();
        java.util.Set<Long> activeFundingEventIds = fundingEventRepository.findAll().stream()
        ...
        long discoveredEvents = fundingEventRepository.findAll().stream()
                .filter(event -> event.getStatus() == FundingEventStatus.DISCOVERED)
                .count();
        long armedTrades = armedTradeRepository.findAll().stream()
        ...
        Map<String, VenueRequestTimingService.Snapshot> latestTimings = venueDiagnosticsService.listTimings().stream()
        ...
        Map<String, Long> totalArmedByVenue = armedTradeRepository.countArmedTradesByVenue(ACTIVE_TRADE_STATES).stream()
                .collect(Collectors.toMap(
                        row -> ((String) row[0]).trim().toLowerCase(Locale.ROOT), row -> (Long) row[1]));
        Map<String, Long> enrichedArmedByVenue =
                liquidityAssessmentRepository.countEnrichedArmedTradesByVenue(ACTIVE_TRADE_STATES).stream()
                        .collect(Collectors.toMap(
                                row -> ((String) row[0]).trim().toLowerCase(Locale.ROOT), row -> (Long) row[1]));
        ...
        List<VenueBlock> venues = venueSummaries.stream()
                ...
                .map(venue -> {
                    ...
                    Double coveragePct = (total == null || total == 0L) ? null : enriched * 100.0 / total;
        ...
        long activeVenues = venueSummaries.stream()
                .filter(VenueDiagnosticsService.VenueSummary::enabledForMetadata)
                .count();
    ...
    private EnrichmentFreshnessSnapshot computeEnrichmentFreshness() {
        ...
        List<Long> activeTradeIds =
                activeTrades.stream().map(ArmedTradeEntity::getId).collect(Collectors.toList());
