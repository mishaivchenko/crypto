...
public class EnginePlanStatusCalculator {
    ...
    public EnginePlanStatusCalculator(
            MonitorEnginePlanProperties engineProperties, EngineEntryAttemptScheduleBuilder attemptScheduleBuilder) {
    ...
    public EnginePlanStatus deriveStatus(ArmedTrade trade, Instant now) {
        ...
        if (trade.state() == ArmedTradeState.ARMED
                || trade.state() == ArmedTradeState.ENTRY_PENDING
                || trade.state() == ArmedTradeState.ENTRY_ATTEMPTED) {
