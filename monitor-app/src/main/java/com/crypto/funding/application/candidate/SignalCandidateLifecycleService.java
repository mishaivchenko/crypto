...
public class SignalCandidateLifecycleService {
    ...
    public SignalCandidate deleteCandidate(Long candidateId, String note) {
        SignalCandidateEntity candidate = signalCandidateRepository
                .findById(candidateId)
                .orElseThrow(() -> new ResourceNotFoundException("Сигнал не найден: " + candidateId));
    ...
    private void removeLinkedPipeline(SignalCandidateEntity candidate) {
        FundingEventEntity fundingEvent = fundingEventRepository
                .findBySignalCandidateId(candidate.getId())
                .orElse(null);
        if (fundingEvent == null && candidate.getFundingEventId() != null) {
            fundingEvent = fundingEventRepository
                    .findById(candidate.getFundingEventId())
                    .orElse(null);
        }
        ...
        for (ArmedTradeEntity armedTrade :
                armedTradeRepository.findAllByFundingEventIdOrderByCreatedAtDesc(fundingEvent.getId())) {
            tradeJournalEntryRepository.deleteAllByEntityTypeAndEntityId(
                    TradeJournalEntityType.ARMED_TRADE, armedTrade.getId());
            armedTradeRepository.delete(armedTrade);
        }

        tradeJournalEntryRepository.deleteAllByEntityTypeAndEntityId(
                TradeJournalEntityType.FUNDING_EVENT, fundingEvent.getId());
