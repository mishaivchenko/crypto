...
public class OperatorAccountService implements ApplicationRunner {
    ...
    public OperatorAccountService(OperatorSecurityProperties properties, OperatorAccountJpaRepository repository) {
    ...
    public void run(ApplicationArguments args) {
        if (properties.getBootstrapUsers() == null
                || properties.getBootstrapUsers().isBlank()) {
            return;
        }
    ...
    public Optional<OperatorPrincipal> authenticate(String rawToken) {
        ...
        return repository
                .findByTokenHashAndEnabledTrue(tokenHash)
    ...
    private void upsertBootstrapUser(String rawEntry) {
        ...
        if (parts.length != 2 || parts[0].isBlank() || parts[1].isBlank()) {
            throw new IllegalStateException(
                    "Invalid SECURITY_OPERATOR_BOOTSTRAP_USERS entry. Expected username:token.");
        }
