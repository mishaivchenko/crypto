...
from quality_backlog.github_client import GitHubError
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not github_client.ensure_labels(all_labels, repo):
        return 1
    ...
    for group in selected:
        ...
        if existing:
            if github_client.add_comment(repo, existing, renderer.render_recurrence_comment(group)):
                ...
                failed += 1
                print(f"[quality-backlog] ERROR: failed to update existing issue #{existing} for {group.fingerprint}")
            continue
        ...
        if issue_number:
            ...
            failed += 1
            print(f"[quality-backlog] ERROR: failed to create issue for {group.fingerprint}")

    print(f"[quality-backlog] Create mode complete: created={created}, updated={updated}, failed={failed}")
