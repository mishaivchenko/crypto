...
public class CommandRouter {
    ...
    public List<BaseRequest<?, ?>> routeMessage(Message message) {
        ...
        if (isAccessDenied(userId)) {
            return List.of(new SendMessage(chatId, MessageFormatter.accessDenied()).parseMode(ParseMode.MarkdownV2));
        }
        ...
        return switch (command) {
            ...
            case "/help" ->
                List.of(new SendMessage(chatId, MessageFormatter.help())
    ...
    public List<BaseRequest<?, ?>> routeCallback(CallbackQuery callbackQuery) {
        ...
        if (isAccessDenied(userId)) {
            return List.of(
                    ack, new SendMessage(chatId, MessageFormatter.accessDenied()).parseMode(ParseMode.MarkdownV2));
        }
    ...
    private InlineKeyboardMarkup buildMainKeyboard() {
        return new InlineKeyboardMarkup(
                new InlineKeyboardButton[] {
                    new InlineKeyboardButton("📚 FAQ").callbackData("menu:faq"),
                    new InlineKeyboardButton("🔗 Ссылки").callbackData("menu:links")
                },
                new InlineKeyboardButton[] {
                    new InlineKeyboardButton("📊 Статус").callbackData("menu:status"),
